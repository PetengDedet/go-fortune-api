...
func (app *PublishedPostApp) GetLatestPostHomepageByContentTypeSLug(contentTypeSlug string) (pl []entity.PostList, e error) {
...
func (app *PublishedPostApp) GetLatestPostByContentTypeSLug(contentTypeSlug string, page int) (pl []entity.PostList, e error) {
	perPage1, err := strconv.Atoi(os.Getenv("LATEST_CONTENT_TYPE_PER_PAGE_1"))
	if err != nil {
		perPage1 = 14
	}

	perPage2, err := strconv.Atoi(os.Getenv("LATEST_CONTENT_TYPE_PER_PAGE_2"))
	if err != nil {
		perPage2 = 9
	}

	maxPage, err := strconv.Atoi(os.Getenv("LATEST_CONTENT_TYPE_MAX_PAGE"))
	if err != nil {
		maxPage = 10
	}
	...
	posts, err := app.PublishePostRepo.GetLatestPublishedPostByPostTypeId(take, skip, contentType.ID)
	if err != nil {
		return nil, err
	}
...
func (app *PublishedPostApp) GetLatestPostByCategorySLug(categorySlug string, page int) (pl []entity.PostList, e error) {
	perPage1, err := strconv.Atoi(os.Getenv("LATEST_CATEGORY_PER_PAGE_1"))
	if err != nil {
		perPage1 = 9
	}

	perPage2, err := strconv.Atoi(os.Getenv("LATEST_CATEGORY_PER_PAGE_2"))
	if err != nil {
		perPage2 = 9
	}

	maxPage, err := strconv.Atoi(os.Getenv("LATEST_CATEGORY_MAX_PAGE"))
	if err != nil {
		maxPage = 10
	}
	...
	category, err := app.CategoryRepo.GetCategoryBySlug(categorySlug)
	if err != nil {
		return nil, err
	}

	posts, err := app.PublishePostRepo.GetLatestPublishedPostByCategoryId(take, skip, category.ID.Int64)
	if err != nil {
		return nil, err
	}
