...
func (repo *PublishedPostRepo) GetLatestPublishedPostByPostTypeId(limit, skip int, postTypeId int64) ([]entity.PostList, error) {
	query := `
		SELECT
			pp.id, 
			pp.title,
	...
	rows, err := repo.DB.Query(query, postTypeId, limit, skip)
	if err != nil {
		return nil, err
	}
